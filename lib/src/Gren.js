...
class Gren {
    ...
    _templateReleases(releases) {
        ...
        return releases.map(release => generate({
            ...
            date: this._formatDate(new Date(release.published_at)),
            body: release.body
        ...
        const ranges = await Promise.all(
            releaseRanges
                .map(async range => {
                    ...
                    this.tasks[taskName].text = `Get commits between ${this._formatDate(new Date(since))} and ${this._formatDate(new Date(until))}`;
    ...
    _formatDate(date) {
        return new Intl.DateTimeFormat(
            this.options.template.dateTimeFormat.locales,
            this.options.template.dateTimeFormat.options
        ).format(date);
    }
